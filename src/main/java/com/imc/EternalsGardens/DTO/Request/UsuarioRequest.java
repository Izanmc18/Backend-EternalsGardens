...
@Schema(
        description = "Datos para registro y actualización de usuario",
        example = "{\n" +
                "  \"nombre\": \"Juan\",\n" +
...
public class UsuarioRequest {
    ...
    @Pattern(
            regexp = "^[0-9]{8}[A-Z]$",
            message = "Formato de DNI inválido (8 números + letra)"
    )
    @Schema(
            description = "DNI del usuario (8 dígitos + letra mayúscula)",
            example = "12345678Z",
            pattern = "^[0-9]{8}[A-Z]$"
    )
    private String dni;
    ...
    @Schema(
            description = "Contraseña del usuario (mínimo 8 caracteres, se encriptará)",
            example = "password123",
            minLength = 8
    )
    ...
    @Pattern(
            regexp = "^[6-7][0-9]{8}$",
            message = "Formato de teléfono inválido (9 dígitos empezando por 6 o 7)"
    )
    @Schema(
            description = "Teléfono móvil del usuario (formato español: 9 dígitos)",
            example = "612345678",
            pattern = "^[6-7][0-9]{8}$"
    )
    ...
    @Schema(
            description = "Fecha de nacimiento del usuario (debe ser en el pasado)",
            example = "1985-03-15",
            type = "string",
            format = "date"
    )
    ...
    @Schema(
            description = "ID del rol del usuario (1=Admin, 2=Operador, 3=Usuario)",
            example = "1",
            minimum = "1"
    )
